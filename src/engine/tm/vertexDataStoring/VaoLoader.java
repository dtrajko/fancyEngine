package engine.tm.vertexDataStoring;
...
import engine.tm.openglObjects.Attribute;
 ...
 * want - it makes no difference to the outcome of the tutorial. Just get those
 * vertex attributes to the vertex shader in whatever way you want! I'll
 * probably talk about these packed formats in a future tutorial.
 * 
 * @author Karl
 *
 */
public class VaoLoader {
	...
	public static Vao createWaterVao(byte[] meshData) {
		...
		vao.initDataFeed(buffer, GL15.GL_STATIC_DRAW, new Attribute(0, GL11.GL_FLOAT, 2), new Attribute(1, GL11.GL_BYTE, 4));
		vao.unbind();
		return vao;
	}
