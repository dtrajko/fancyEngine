 ...
 *
 */
public class WaterMeshRenderer {

	private static final float WAVE_SPEED = 0.002f;

	private final WaterMeshShader shader;
	 ...
	 */
	public WaterMeshRenderer() {
		this.shader = new WaterMeshShader();
	}
	 ...
	 */
	public void render(WaterMesh water, ThinMatrixCamera camera, Light light, int reflectionTexture, int refractionTexture,
			int depthTexture) {
	 ...
	 *            - The scene's light.
	 */
	private void prepare(WaterMesh water, ThinMatrixCamera camera, Light light) {
		water.getVao().bind();
	...
	private void bindTextures(int reflectionTexture, int refractionTexture, int depthTexture) {
		bindTextureToUnit(reflectionTexture, WaterMeshShader.REFLECT_TEX_UNIT);
		bindTextureToUnit(refractionTexture, WaterMeshShader.REFRACT_TEX_UNIT);
		bindTextureToUnit(depthTexture, WaterMeshShader.DEPTH_TEX_UNIT);
	}
	 ...
	 */
	private void finish(WaterMesh water) {
		water.getVao().unbind();
		shader.stop();
	 ...
	 *            - The light in the scene.
	 */
	private void prepareShader(WaterMesh water, ThinMatrixCamera camera, Light light) {
		shader.start();
		updateTime();
