...
import engine.tm2.opengl.Query;
...
public class FlareRenderer {
	...
	private static float TEST_QUAD_HEIGHT;
	private static float TOTAL_SAMPLES;
	 ...
	 */
	public FlareRenderer(Window window) {
		
		TEST_QUAD_HEIGHT = TEST_QUAD_WIDTH * window.getWidth() / window.getHeight();
		TOTAL_SAMPLES = (float) Math.pow(TEST_QUAD_WIDTH * window.getWidth() * 0.5f, 2) * 4;
	 ...
	 *            at.
	 */
	public void render(Window window, Vector2f sunScreenCoords, FlareTexture[] flares, float brightness) {
		prepare(brightness);
		doOcclusionTest(sunScreenCoords);
		OpenGlUtils.enableAdditiveBlending();
		OpenGlUtils.enableDepthTesting(false);
		for (FlareTexture flare : flares) {
			renderFlare(window, flare);
		}
		endRendering();
	}
	 ...
	 */
	private void renderFlare(Window window, FlareTexture flare) {
